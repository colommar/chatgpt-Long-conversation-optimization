...
- 任何“定位 / 搜索 / 时间线跳转”都只能基于“当前已加载或已缓存到本地会话状态”的消息执行。
- 当目标消息尚未渲染到页面时，功能可能需要先触发滚动加载，或者暂时无法一次到位。
...
- ✅ 导出 JSON 可用（优先使用已见消息缓存）
- ✅ Prompt 指令库（增删改查、导入导出、复制提示）可用
- ✅ 对话文件夹管理（创建、重命名、删除、归类、排序）可用
- ✅ 多语言切换、主题同步、面板拖拽可用
- ✅ 搜索可用（基于已加载+缓存消息，虚拟化场景下会给出加载提示）
- ⚠️ 时间线基础渲染、节点增长、视口高亮同步：可用
- ❌ 时间线“点击节点跳转”当前不稳定（已确认问题，正在修复中）

> 说明：若你在长会话中需要更稳定的定位体验，建议先滚动到目标区间附近，让 ChatGPT 先把该段消息渲染出来，再执行跳转/搜索。
...
- 🕒 对话时间线：基于当前页面已加载的用户消息生成时间节点，支持预览、节点计数、显示/隐藏、拖拽移动（点击节点跳转当前不稳定，修复中）。
